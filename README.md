...
| C. AI 视觉测量 | LabelMe 曲线监督、TensorFlow U-Net 分割、上下分支中心线重建、饱和合并区约束平滑、逐图横纵网格检测，并在同一重建曲线上测量 Hc/Br/饱和端点。 |
| D. Web 教学与分析 | Streamlit 交互式上传、完整分割回线绘图、关键点复核、回线面积/单位体积单周期磁滞损耗计算、失败回退和教学反馈展示；结果图以浅色填充积分区域、用箭头标示上下支方向并显示对应公式，采用响应式 1:1 绘图区和适配移动端的放大字号。 |
