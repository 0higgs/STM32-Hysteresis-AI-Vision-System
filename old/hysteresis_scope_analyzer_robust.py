...
def imread_unicode(path: str
    ...
    """兼容中文路径读取图像。"""
    data = np.fromfile(path, dtype=np.
uint8)
    img = cv2.imdecode(data, cv2.
IMREAD_COLOR)
    return img
...
def make_json_safe(obj
    ...
    if isinstance(obj, dict
        ...
        return {str(k): make_json_safe(v) for k, v in obj.
items()}
...
float32)
    ...
    # 大尺度背景估计，突出细亮线
    g_u8 = np.clip(g, 0, 255).astype(np.
uint8)
    ...
    tophat = cv2.subtract(g_u8, opened).astype(np.
float32)

    # HSV约束，减少蓝色背景和暗网格影响
    hsv = cv2.cvtColor(roi_bgr, cv2.
COLOR_BGR2HSV)
    H, S, V = cv2.split(hsv
)
    hsv_gate = ((H >= 35) & (H <= 95) & (S >= 25) & (V >= 80)).astype(np.
float32)
    ...
    score_u8 = cv2.normalize(score, None, 0, 255, cv2.NORM_MINMAX).astype(np.
uint8)
    return score_u8
...
:
        # 先用 Otsu
        _, m1 = cv2.threshold(score_u8, 0, 255, cv2.THRESH_BINARY + cv2.
THRESH_OTSU)
...
:
        ...
        _, m2 = cv2.threshold(score_u8, max(10, q), 255, cv2.
THRESH_BINARY)
...
:
        ...
        _, m = cv2.threshold(score_u8, th, 255, cv2.
THRESH_BINARY)
        return m
...
:
        ...
        mean_score = float(score_u8[comp_mask].
mean())
...
:
    ...
    return xs.astype(np.float64), ys.astype(np.
float64)
...
:
    ...
    px_per_div_x = w / float(args.
x_divs)
    px_per_div_y = h / float(args.
y_divs)
    ...
    UH = (xs - ox) / px_per_div_x * float(args.
x_vdiv)
    UB = -(ys - oy) / px_per_div_y * float(args.
y_vdiv)
...
:
        H = args.n * UH / (args.l * args.
R1)
    if args.S is not None
:
        B = args.R2 * args.C2 * UB / (args.N * args.
S)
    return H, B
...
def write_report(out_path, image_name, args, params, diagnostics
 ...
 []
    ...
    for k, v in params.get("normalized", {}).
items():
        lines.append(f"- {k}: {v}"
)
    lines.append(""
...
:
        ...
        for k, v in params.get("physical", {}).
items():
            lines.append(f"- {k}: {v}"
)
    lines.append(""
...
:
    ...
    robust_mkdir(args.
outdir)
    stem = Path(args.image).
stem

    roi_given = parse_roi(args.
roi)
...
:
    ...
    # 提取轨迹
    mask, score, comps = extract_trace_mask(roi, threshold=args.threshold, min_area=args.
min_area)
...
(
        Hs, Bs, bins=args.bins, smooth_window=args.
smooth_window,
        min_points_per_bin=args.
min_points_per_bin
    )
...
(
        UH, UB, bins=args.bins, smooth_window=args.
smooth_window,
        min_points_per_bin=args.
min_points_per_bin
    )
...
,
    ...
    branches["U_H_V"] = x_v[:len(branches)] if len(x_v) == len(branches) else np.
nan
...
(
            H_phys, B_phys, bins=args.bins, smooth_window=args.
smooth_window,
            min_points_per_bin=args.
min_points_per_bin
        )
